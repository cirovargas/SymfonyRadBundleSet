...
namespace Core\Bundle\AlertaBundle\Controller;
...
class DefaultController extends Controller
{
    public function containerAction(){
        ...
        //$em->flush();
        $messages = array();
        $qb = $em->getRepository('CoreAlertaBundle:UserAlerta')->createQueryBuilder('u');
        $qb = $qb
            ->innerJoin('u.alert','a')
            ->addSelect('a')
            ->where('u.user = :user')
        ...
        $qCount = $qCount->select('count(u.id)')
                ->from('CoreAlertaBundle:UserAlerta','u')
        ...
        return $this->render('CoreAlertaBundle:Default:container.html.twig',array(
            'messages' => $messages,
            'count' => $count
        ));
    }
    ...
    public function allAction(){
        ...
        return $this->render('CoreAlertaBundle:Default:all.html.twig',array(
            'messages' => $messages
        ));
    }
    ...
    public function setReadedAction(Request $request){
        ...
        $em = $this->getDoctrine()->getManager();
        $notifications = $em->getRepository('CoreAlertaBundle:UserAlerta')->findBy(array('id'=>$request->request->get('notificationIds')));
        $ids = array();
